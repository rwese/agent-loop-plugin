...
export function createTaskContinuation(ctx, options = {}) {
    const { countdownSeconds = 2, errorCooldownMs = 3000, toastDurationMs = 900, agent, model, logFilePath, } = options;
    ...
    const handleSessionError = async (sessionID, event) => {
        ...
        if (error instanceof Error) {
            ...
            isInterruption = !!(errorObj.name?.includes("Abort") ||
    ...
    function extractSessionID(event) {
        ...
        if (props?.info?.id && typeof props.info.id === "string")
